...
// Redefine the nodes and base edges safely
export const BAHIR_DAR_NODES = [
...
async function getRoute(sourceNode, targetNode) {
  const url = `http://router.project-osrm.org/route/v1/driving/${sourceNode.lng},${sourceNode.lat};${targetNode.lng},${targetNode.lat}?geometries=geojson&overview=full`;
  try {
    const res = await fetch(url);
    const data = await res.json();
    if (data.routes && data.routes.length > 0) {
      // route geometries are in lng, lat
      const coords = data.routes[0].geometry.coordinates;
      // We only want the intermediate waypoints, skip the first and last (since those are node coords)
      if (coords.length <= 2) return undefined;
      const waypoints = coords.slice(1, coords.length - 1).map(c => [c[1], c[0]]);
      return waypoints; // lat, lng
    }
  } catch (err) {
    console.error(`Error fetching route from ${sourceNode.id} to ${targetNode.id}:`, err);
  }
  return undefined;
}
...
async function main() {
  const updatedEdges = [];
  console.log('Fetching true paths for edges...');
  
  for (let i = 0; i < BASE_EDGES.length; i++) {
    const edge = BASE_EDGES[i];
    const sourceNode = BAHIR_DAR_NODES.find(n => n.id === edge.source);
    const targetNode = BAHIR_DAR_NODES.find(n => n.id === edge.target);
    
    if (sourceNode && targetNode) {
      const waypoints = await getRoute(sourceNode, targetNode);
      if (waypoints && waypoints.length > 0) {
        updatedEdges.push({ ...edge, waypoints });
      } else {
        updatedEdges.push(edge);
      }
      console.log(`Fetched route ${i+1}/${BASE_EDGES.length}: ${sourceNode.id} -> ${targetNode.id} (${waypoints ? waypoints.length : 0} waypoints)`);
    } else {
      updatedEdges.push(edge);
    }
    // Respect OSRM public API rate limits (1 request per second is very safe)
    await sleep(200);
  }
  
  console.log('Finished fetching. Generating ts file...');
  
  const tsContent = `import { Node, Edge } from '../types/index';

export const BAHIR_DAR_NODES: Node[] = ${JSON.stringify(BAHIR_DAR_NODES, null, 2)};
...
const BASE_EDGES: Edge[] = ${JSON.stringify(updatedEdges, null, 2).replace(/"([^"]+)":/g, '$1:')};

export const BAHIR_DAR_EDGES: Edge[] = createBidirectional(BASE_EDGES);
`;

  fs.writeFileSync(path.join(process.cwd(), 'src/data/bahirdar-graph.ts'), tsContent);
  console.log('Successfully updated src/data/bahirdar-graph.ts');
}
